 ...
 public:
  static rtc::scoped_refptr<CapturerTrackSource> Create() {
    ...
    capturer = absl::WrapUnique(
      webrtc::WrappedDesktopCapturer::Create());
    if (capturer) {
...
void Conductor::OnMessageFromPeer(int peer_id, const std::string& message) {
  ...
  RTC_LOG(LS_INFO) << "!!!Received:" << peer_id << ": "
                   << message;
  ...
  if (!type_str.empty()) {
    ...
    RTC_LOG(LS_INFO) << "che3 2";
    webrtc::SdpParseError error;
    std::unique_ptr<webrtc::IceCandidateInterface> candidate(
        webrtc::CreateIceCandidate(sdp_mid, sdp_mlineindex, sdp, &error));
    RTC_LOG(LS_INFO) << "che3 3";    
    if (!candidate.get()) {
    ...
    RTC_LOG(LS_INFO) << "che3 4";
    if (!peer_connection_->AddIceCandidate(candidate.get())) {
      RTC_LOG(LS_WARNING) << "Failed to apply the received candidate";
      return;
    }
    RTC_LOG(LS_INFO) << "che3 444";
    RTC_LOG(LS_INFO) << " Received candidate :" << message;
    RTC_LOG(LS_INFO) << "che3 5";
  }
}
...
void Conductor::UIThreadCallback(int msg_id, void* data) {
  RTC_LOG(LS_INFO) << __FUNCTION__ << " " << msg_id;
  switch (msg_id) {
    ...
    case SEND_MESSAGE_TO_PEER: {
      ...
      if (msg) {
        ...
        RTC_LOG(LS_INFO) << "SEND_MESSAGE_TO_PEER: " << *msg;
    ...
    case NEW_TRACK_ADDED: {
      RTC_LOG(LS_INFO) << "NEW_TRACK_ADDED";
